...
class UpdatePemesananRequest extends FormRequest
{
    ...
    protected $errorBag = 'update'; // <-- TAMBAHKAN BARIS INI
    ...
    public function messages()
    {
        return [
            ...
            // 'status.in' => 'Nilai status yang dipilih tidak valid.',
            'ruangan_id.required'    => 'Ruangan wajib dipilih.',
            'nama_kegiatan.required' => 'Nama kegiatan wajib diisi.',
            'waktu_mulai.required'   => 'Waktu mulai wajib diisi.',
            'waktu_selesai.required' => 'Waktu selesai wajib diisi.',
    ...
    {
        session()->flash('failed_update_id', $this->route('pemesanan')->id);
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Ambil data waktu yang SUDAH digabung oleh prepareForValidation
            $startDateTime = $this->input('waktu_mulai');
            $endDateTime = $this->input('waktu_selesai');

            $pemesananIdToExclude = $this->route('pemesanan')->id;

            if ($ruanganId && $startDateTime && $endDateTime) {
                // Cek apakah ada pemesanan lain yang tumpang tindih
                $isConflict = Pemesanan::where('ruangan_id', $ruanganId)
                    ->where('id', '!=', $pemesananIdToExclude)
